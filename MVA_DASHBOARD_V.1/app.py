...
def check_password():
    ...
    if not expected_password:
        expected_password = "default_pass"  # Fallback for dev
...
try:
    if check_password():
        ...
        # ✅ AUTO-LAUNCH DASHBOARD IF ANALYSIS IS LOADED
        if 'loaded_analysis' in st.session_state:
            data = st.session_state['loaded_analysis']
            data_processor_10th.create_dashboard(data['df'], data['results'], data['models'])
            st.stop()

        # --- Load Past Runs from DB ---
        ...
        if not past_runs.empty:
            ...
            selected_run = st.selectbox(
                "Select a previous analysis run:",
        ...
        # --- Upload New Data (only shown if no analysis loaded) ---
        ...
        if class_group == "8th to 10th":
            st.header("Upload 8th to 10th Data")
            ...
            if uploaded_file is not None:
                with st.spinner("Processing uploaded file..."):
                    ...
                    try:
                        ...
                        df_parsed, metadata = data_processor_10th.load_and_parse_10th_sheet(temp_file_path)

                        if df_parsed.empty:
                            st.error("❌ No student data parsed.")
                        else:
                            st.success(f"✅ Parsed {len(df_parsed)} students.")

                            # Generate hash based on academic columns only
                            ...
                            if existing_run:
                                st.warning(f"⚠️ This dataset already exists in the database (Run ID: {existing_run}).")
                                ...
                                if not overwrite:
                                    ...
                                    if df_loaded is not None:
                                        data_processor_10th.create_dashboard(df_loaded, results_loaded, models_loaded)
                                        st.stop()
                                    else:
                                        st.error("Failed to load existing data. Proceeding with re-analysis.")
                            ...
                            # Save to SQL (primary persistence)
                            with st.spinner("Saving to database..."):
                                data_processor_10th.process_and_save_to_sql(
                                    df_parsed, analysis_results, trained_models, db_config, data_hash=new_data_hash
                                )
                            st.success("✅ Data and analysis saved to database.")

                            # Launch dashboard
                            data_processor_10th.create_dashboard(df_parsed, analysis_results, trained_models)
                            st.stop()
